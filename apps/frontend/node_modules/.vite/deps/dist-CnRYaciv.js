...
import { $ as safeParseJSON, A as getRuntimeEnvironmentUserAgent, At as AISDKError, Bt as NoSuchModelError, Ct as number, D as filterNullable, Dt as string, E as fetchWithValidatedRedirects, Et as strictObject, F as isExecutableTool, Gt as getErrorMessage, H as loadOptionalSetting, I as isFullMediaType, It as JSONParseError, J as postJsonToApi, K as parseTranscriptionStreamPart, Kt as isJSONArray, L as isNonNullable, M as isAbortError, N as isBuffer, Nt as InvalidArgumentError$1, O as generateId, Ot as union, Pt as InvalidPromptError, Q as retryWithExponentialBackoff, R as isProviderReference, S as detectMediaType, St as never, T as executeTool, Tt as record, U as normalizeHeaders, Ut as TypeValidationError, V as lazySchema, W as parseJsonEventStream, Wt as UnsupportedFunctionalityError, X as resolve, Y as readResponseWithSizeLimit, _ as createIdGenerator, _t as custom, a as TRANSCRIPTION_STREAM_AUDIO_DONE_FRAME_TYPE, at as waitForWebSocketBufferDrain, b as createProviderExecutedToolFactory, bt as literal, c as asSchema, ct as zodSchema, d as connectToWebSocket, dt as _enum, et as safeValidateTypes, f as convertBase64ToUint8Array, ft as _instanceof, g as createEventSourceResponseHandler, gt as boolean, h as convertUint8ArrayToBase64, ht as array$1, it as validateTypes, jt as APICallError, k as getFromApi, kt as unknown, l as cancelResponseBody, lt as WORKFLOW_DESERIALIZE, mt as any, n as DelayedPromise, nt as serializeModelOptions, o as TRANSCRIPTION_STREAM_START_FRAME_TYPE, ot as withUserAgentSuffix, pt as _null, qt as isJSONObject, r as DownloadError, s as asArray, st as withoutTrailingSlash, t as DEFAULT_MAX_DOWNLOAD_SIZE, tt as secureJsonParse, u as combineHeaders, ut as WORKFLOW_SERIALIZE, v as createJsonErrorResponseHandler, vt as discriminatedUnion, wt as object$1, x as delay, y as createJsonResponseHandler, yt as lazy, z as isUrlSupported } from "./dist-CUxlP58J.js";
...
function resolveTranscriptionModel(model) {
	var _a22, _b;
	if (typeof model === "string") return (_b = (_a22 = getGlobalProvider()).transcriptionModel) == null ? void 0 : _b.call(_a22, model);
	if (![
		"v4",
		"v3",
		"v2"
...
function resolveSpeechModel(model) {
	var _a22, _b;
	if (typeof model === "string") return (_b = (_a22 = getGlobalProvider()).speechModel) == null ? void 0 : _b.call(_a22, model);
	if (![
		"v4",
		"v3",
		"v2"
...
function resolveVideoModel(model) {
	...
	if (typeof model === "string") {
		const videoModel = ((_a22 = globalThis.AI_SDK_DEFAULT_PROVIDER) != null ? _a22 : gateway).videoModel;
...
function streamObject(options) {
	const { model, output = "object", instructions, system, prompt, messages, allowSystemInMessages, maxRetries, abortSignal, headers, experimental_repairText: repairText, experimental_telemetry, telemetry = experimental_telemetry, experimental_download: download2, providerOptions, onStart, experimental_onStart, onStepStart, experimental_onStepStart, onStepEnd, onStepFinish, onError = ({ error }) => {
		console.error(error);
...
async function fingerprintTools(tools) {
	const entries = await Promise.all(Object.keys(tools).map(async (name22) => {
		const tool2 = tools[name22];
		return [name22, await hashCanonical({
...
function detectToolDrift(current, baseline) {
	...
	const removed = [];
	const changed = [];
	for (const name22 of Object.keys(current)) if (!Object.hasOwn(baseline, name22)) added.push(name22);
	else if (current[name22] !== baseline[name22]) changed.push(name22);
	for (const name22 of Object.keys(baseline)) if (!Object.hasOwn(current, name22)) removed.push(name22);
	return {
		added,
		removed,
		changed
...
var BrowserRealtimeAudio = class {
	...
	stopCapture() {
		var _a22, _b, _c, _d;
		(_a22 = this.captureProcessor) == null || _a22.disconnect();
	...
	dispose() {
		...
		this.stopCapture();
		this.stopPlayback();
		(_a22 = this.playbackContext) == null || _a22.close();
		this.playbackContext = null;
	}
	setPlaying(isPlaying) {
		var _a22, _b;
		if (isPlaying && !this.isPlaying) this.playbackStartTime = (_b = (_a22 = this.playbackContext) == null ? void 0 : _a22.currentTime) != null ? _b : 0;
...
var BrowserRealtimeTransport = class {
	...
	sendRaw(data) {
		var _a22;
		if (((_a22 = this.ws) == null ? void 0 : _a22.readyState) === WebSocket.OPEN) if (typeof data === "string" || data instanceof ArrayBuffer || ArrayBuffer.isView(data) || data instanceof Blob) this.ws.send(data);
...
var RealtimeEventReducer = class {
	...
	addInputTranscriptionMessage(state, itemId, transcript) {
		...
		const insertIndex = Math.min((_a22 = this.inputAudioMessageInsertIndex.get(itemId)) != null ? _a22 : state.messages.length, state.messages.length);
	...
	appendTextDelta(state, itemId, delta) {
		...
		const text2 = ((_a22 = this.textAccumulators.get(itemId)) != null ? _a22 : "") + delta;
...
var AbstractRealtimeSession = class {
	constructor(options) {
		...
		this.transport = new BrowserRealtimeTransport({
			...
			onError: (error) => {
				...
				(_a23 = this.onError) == null || _a23.call(this, error);
			},
			onClose: () => {
	...
	async connect() {
		...
		try {
			...
			this.applyState(this.reducer.setStatus(this.state, "error"));
			(_a22 = this.onError) == null || _a22.call(this, error instanceof Error ? error : /* @__PURE__ */ new Error(`Connection failed: ${String(error)}`));
		}
	}
	disconnect() {
	...
	*/
	...
	async executeToolCall({ name: name22, args, callId }) {
		var _a22, _b;
		if (this.onToolCall == null) {
			(_a22 = this.onError) == null || _a22.call(this, /* @__PURE__ */ new Error(`No handler provided for tool "${name22}"`));
			return;
		}
		try {
	...
	async handleServerEvent(event) {
		...
		(_a22 = this.onEvent) == null || _a22.call(this, event);
	...
	handleReducerEffect(effect) {
		...
		switch (effect.type) {
			...
			case "tool-call":
				...
				break;
			case "error":
				(_a22 = this.onError) == null || _a22.call(this, effect.error);
				break;
		}
	}
};
...
async function uploadFile({ api, data: dataArg, mediaType: mediaTypeArg, filename, providerOptions }) {
	...
	const mediaType = mediaTypeArg != null ? mediaTypeArg : data.type === "text" ? "text/plain" : (_a22 = detectMediaType({ data: data.data })) != null ? _a22 : isLikelyText(data.data) ? "text/plain" : "application/octet-stream";
...
//#endregion
export { defaultSettingsMiddleware as $, simulateStreamingMiddleware as $t, ToolCallRepairError as A, isFileUIPart as At, convertFileListToFileUIParts as B, modelMessageSchema as Bt, NoSuchToolError as C, getToolTimeoutMs as Ct, SerialJobExecutor as D, isDataUIPart as Dt, RetryError as E, isCustomContentUIPart as Et, addToolInputExamplesMiddleware as F, isStepCount as Ft, createDownload as G, pipeUIMessageStreamToResponse as Gt, cosineSimilarity as H, parsePartialJson as Ht, assistantModelMessageSchema as I, isTextUIPart as It, createUIMessageStream as J, registerTelemetry as Jt, createProviderRegistry as K, pruneMessages as Kt, callCompletionApi as L, isToolUIPart as Lt, UIMessageStreamError as M, isReasoningFileUIPart as Mt, UI_MESSAGE_STREAM_HEADERS as N, isReasoningUIPart as Nt, TextStreamChatTransport as O, isDeepEqualData as Ot, UnsupportedModelVersionError as P, isStaticToolUIPart as Pt, defaultEmbeddingSettingsMiddleware as Q, simulateReadableStream as Qt, consumeStream as R, lastAssistantMessageIsCompleteWithApprovalResponses as Rt, NoSuchProviderError as S, getToolOrDynamicToolName as St, NoVideoGeneratedError as T, hasToolCall as Tt, createAgentUIStream as U, pipeAgentUIStreamToResponse as Ut, convertToModelMessages as V, output_exports as Vt, createAgentUIStreamResponse as W, pipeTextStreamToResponse as Wt, customProvider as X, resampleAudio as Xt, createUIMessageStreamResponse as Y, rerank as Yt, decodeRealtimeAudio as Z, safeValidateUIMessages as Zt, MissingToolResultsError as _, wrapImageModel as _n, getRealtimeToolDefinitions as _t, DefaultGeneratedFile as a, systemModelMessageSchema as an, experimental_generateSpeech as at, NoOutputGeneratedError as b, createGateway as bn, getTextFromDataUrl as bt, InvalidArgumentError as c, toUIMessageStream as cn, extractJsonMiddleware as ct, InvalidStreamPartError as d, uiMessageChunkSchema as dn, fingerprintTools as dt, smoothStream as en, detectToolDrift as et, InvalidToolApprovalError as f, uploadFile as fn, generateImage as ft, MessageConversionError as g, wrapEmbeddingModel as gn, getChunkTimeoutMs as gt, JsonToSseTransformStream as h, validateUIMessages as hn, generateText as ht, DefaultChatTransport as i, streamTranscribe as in, experimental_createProviderRegistry as it, ToolLoopAgent as j, isLoopFinished as jt, ToolCallNotFoundForApprovalError as k, isDynamicToolUIPart as kt, InvalidDataContentError as l, toolModelMessageSchema as ln, extractReasoningMiddleware as lt, InvalidToolInputError as m, userModelMessageSchema as mn, generateSpeech as mt, AbstractChat as n, streamObject as nn, embedMany as nt, DirectChatTransport as o, toTextStream as on, experimental_generateVideo as ot, InvalidToolApprovalSignatureError as p, uploadSkill as pn, generateObject as pt, createTextStreamResponse as q, readUIMessageStream as qt, AbstractRealtimeSession as r, streamText as rn, encodeRealtimeAudio as rt, HttpChatTransport as s, toUIMessageChunk as sn, experimental_transcribe as st, AI_SDK_TELEMETRY_TRACING_CHANNEL as t, streamLanguageModelCall as tn, embed as tt, InvalidMessageRoleError as u, transcribe as un, filterActiveTools as ut, NoImageGeneratedError as v, wrapLanguageModel as vn, getStaticToolName as vt, NoTranscriptGeneratedError as w, getTotalTimeoutMs as wt, NoSpeechGeneratedError as x, gateway as xn, getToolName as xt, NoObjectGeneratedError as y, wrapProvider as yn, getStepTimeoutMs as yt, convertDataContentToBase64String as z, lastAssistantMessageIsCompleteWithToolCalls as zt };
